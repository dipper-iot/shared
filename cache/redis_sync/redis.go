...
func (r *redisCache) Get(ctx context.Context, key string, data interface{}, options ...cache.OptionCache) (bool, error) {
	...
	timeCache, success := cache.TimeoutFromContext(ctx)
	if !success {
		timeCache = time.Second * 5
	}

	jsonStr, err := r.client.GetEx(ctx, key, timeCache).Result()
	if err != nil {
		return false, nil
	}
...
func (r *redisCache) Set(ctx context.Context, key string, data interface{}, options ...cache.OptionCache) error {
	...
	err = r.client.SetEX(ctx, key, bData, timeCache).Err()
	if err != nil {
		return err
	}
